...
import { ITokenProvider, ITokenValidator, TokenPayload, TokenResponse, TokenValidationResult } from '../interface';
...
export class JwtTokenService implements ITokenProvider {
    ...
    async generateTokens(payload: TokenPayload): Promise<TokenResponse> {
        try {
            ...
            const refreshExpirationMs = this.parseExpiration(this.refreshTokenExpiration) * 1000;
            refreshExpiration.setTime(refreshExpiration.getTime() + refreshExpirationMs);
    ...
    async verifyRefreshToken(token: string): Promise<TokenPayload | null> {
        try {
            ...
            if (!this.isTokenPayload(decoded) || !decoded.jti) {
                this.logger.warn('Decoded refresh token does not match TokenPayload structure or missing jti');
                return null;
            }

            const isWhitelisted = await this.tokenWhitelist.isWhitelisted(decoded.sub, decoded.jti);
            if (!isWhitelisted) {
                this.logger.warn(`Refresh token with jti ${decoded.jti} for user ${decoded.sub} not whitelisted`);
                return null;
            }
    ...
    async invalidateToken(userId: number, tokenId?: string): Promise<void> {
        try {
            this.logger.log(`Invalidating token for user ${userId}, tokenId: ${tokenId || 'all tokens'}`);
            if (tokenId) {
    ...
    private isTokenPayload(payload: unknown): payload is TokenPayload & { jti: string } {
...
export class TokenValidator implements ITokenValidator {
    ...
    async validate(token: string): Promise<TokenValidationResult> {
        try {
            ...
            return { payload: null, error: error as Error, isValid: false };
        }
    }
}